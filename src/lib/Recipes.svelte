...
<div class="flex flex-row h-12 w-full mt-4 mx-4 gap-5 justify-between">
    ...
    <div class="flex flex-row gap-5 h-full w-full">
        <label
            class="input input-bordered flex items-center gap-2 5 w-full bg-base-100"
        >
    ...
    <ul class="menu menu-horizontal w-64 z-50 rounded-md bg-base-100">
        <li class="w-full h-full">
            <details class="w-full h-full">
                <summary
                    class="flex justify-between items-center text-lg w-full h-full"
                >
                    Filters
                </summary>
                <ul class="bg-base-100 w-full rounded-lg">
                    <li class="flex flex-col w-full h-8">
                        <div class="form-control w-full h-full flex">
                            <label
                                class="label cursor-pointer w-full h-full flex justify-between"
                            >
    ...
    <!-- Add Recipe Button -->
    <button
        class="btn btn-square flex-shrink aspect-square hover-grow bg-base-100"
    >
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
...
<div
    class="grid grid-cols-4 gap-5 p-5 overflow-scroll content-center w-screen z-0 pb-[5.25rem]"
...
>
    {#if recipes}
        {#each recipes as recipe}
            ...
            <div
                class="card bg-base-100 shadow-xl image-full hover-grow w-full aspect-square group"
            >
                <figure>
                    <img
                        src={recipe.image}
                ...
                <div class="card-body text-center">
                    ...
                    <div class="card-actions absolute bottom-2 right-2">
                        <button
                            class="btn btn-circle btn-ghost opacity-0 hover-view group-hover:opacity-100 shake"
                            on:click={() => delete_recipe(recipe.id)}
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                    ...
                    <div class="card-actions absolute bottom-2 left-2">
                        <button
                            class="btn btn-circle btn-ghost opacity-0 hover-view group-hover:opacity-100 pulse"
                            on:click={() => showModal(recipe)}
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
...
<dialog id="recipe_model" class="modal">
    <div class="modal-box">
        {#if currentRecipe}
            <h3 class="font-bold text-xl text-center bold pb-5">
                {currentRecipe.name}
            </h3>
            <div class="flex w-auto pb-5 gap-5">
                <!-- Instructions with normal wrapping -->
                <div
                    class="grid h-48 flex-grow card bg-base-300 rounded-box place-items-left p-5"
                    id="ingredients"
                >
                    <ol
                        class="flex flex-col w-full h-full list-decimal overflow-auto list-inside"
                    >
                        {#each currentRecipe.instructions as step}
                            <li class="flex-grow text-left text-xs">{step}</li>
                        {/each}
                    </ol>
                </div>
                <!-- Ingredients with horizontal scrolling -->
                <div
                    class="grid h-48 card bg-base-300 rounded-box place-items-left p-5 hover-target"
                >
                    <div
                        class="flex flex-col w-min min-w-min h-full overflow-x-auto gap-1"
                    >
                        {#each currentRecipe.ingredients as ingredient}
                            <div
                                class="ingredient-item text-left text-xs whitespace-nowrap"
                            >
...
<style>
    ...
    .hover-target:hover .ingredient-details {
